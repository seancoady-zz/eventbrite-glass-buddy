...
class MainHandler(webapp2.RequestHandler):
  ...
  def _insert_item(self):
    ...
    jsonObj = json.load(urllib2.urlopen("http://www.eventbrite.com/directory/json/?lat=37.7750641&lng=-122.3993421&radius=0.5"))

    event_list = []
    for eventObj in jsonObj["events"]:
        event_list.append('<li>' + eventObj["title"] + '</li>')
    jsonObj = '{"html": "<article><section><div class=\"text-normal\"><p style=\"color: #f16924;\">Nearby Events</p><ul class=\"text-x-small\">' + (''.join(event_list)) + '</section></article>"}'
